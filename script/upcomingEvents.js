		...
		checkBoxes.forEach((checkbox) => {
			checkbox.addEventListener('change', () => {
				...
				if (elementos.length == 0) {
					...
					crearCards(filtroArray(upcomingEvents, elementos), containerCard);
					...
					datos1 = filtroArray(upcomingEvents, elementos)
...
//crearCheckBoxes(createCategories(data.events.category), containerCheckBoxes)
crearCheckBoxes(arrCategory, containerCheckBoxes)
let checkBoxes = document.querySelectorAll('input[name="category"]')
let datos1 = []
checkBoxes.forEach((checkbox) => {
...
buscador.addEventListener('keyup', () => {
	...
	datos = filtroArray(upcomingEvents, elementos)
